    ...
    tool: {
      "fixseo.scan": tool({
        ...
        args: {
          ...
          maxPages: tool.schema.number().optional().default(25).describe("Maximum number of pages to crawl"),
          maxDepth: tool.schema.number().optional().default(10).describe("Maximum crawl depth"),
          includeSitemap: tool.schema.boolean().optional().default(true).describe("Whether to use sitemap to discover pages"),
        },
        async execute(args, context) {
          ...
          context.metadata({ 
            ...
            metadata: { 
              ...
              issuesFound: result.summary.high + result.summary.medium + result.summary.low,
            }
          });
