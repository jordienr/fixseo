...
`;
  ...
  const pluginDir = path.join(process.cwd(), ".claude/skills/fixseo");
  const skillPath = path.join(pluginDir, "SKILL.md");

  try {
    await fs.mkdir(pluginDir, { recursive: true });
    await fs.writeFile(skillPath, pluginCode);
    console.log("✅ Claude Code skill installed!");
    console.log(`   Created: ${skillPath}`);
    console.log("");
    console.log("Usage:");
    console.log('   "Check SEO for example.com"');
    console.log('   "Scan my website for issues"');
    console.log("");
    console.log("The skill will be available when you restart Claude Code.");
  } catch (e) {
    console.error("Failed to install Claude Code skill:", e);
    process.exit(1);
  }
}
